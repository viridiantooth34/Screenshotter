...
public class Utilities {
	...
	public void takeWindowScreenShot() {
		...
		try {
			...
			double scalingFactor = screenRect.getWidth() / dm.getWidth();

			// Adjust the screen size based on the scaling factor

			int width = (int) (screenRect.width / scalingFactor);

			int height = (int) (screenRect.height / scalingFactor);

			Rectangle scaledRect = new Rectangle(width, height);

			// Capture the screenshot as a BufferedImage

			BufferedImage screenCapture = robot.createScreenCapture(scaledRect);
	...
	public void copyingImagesFromScreenshotstoWordDocument(ArrayList<String> descriptions1,
			...
			ArrayList<String> descriptions2, String docFileName) {
		...
		try {
			...
			for (File file : folder.listFiles()) {
				...
				if (file.isFile() && file.getName().endsWith(".png")) {
					...
					// Create a new paragraph for Heading 1 if description1 is not empty

					if (index < descriptions1.size() && !descriptions1.get(index).isEmpty()) {
					...
					// Create a new paragraph for Heading 2 if description2 is not empty

					if (index < descriptions2.size() && !descriptions2.get(index).isEmpty()) {
			...
			try (FileOutputStream out = new FileOutputStream(docFileName + ".docx")) {
	...
	public void openWordDocument(String docFileName) {

		String filePath = System.getProperty("user.dir") + "\\" + docFileName + ".docx";
